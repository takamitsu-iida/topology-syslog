  ...
  return (
    <main className="mx-auto max-w-7xl p-4">
      ...
      <section className="overflow-x-auto border bg-white">
        <table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-gray-50 text-xs text-gray-500"><tr><th className="px-3 py-2">受信時刻</th><th className="px-3 py-2">装置</th><th className="px-3 py-2">Severity</th><th className="px-3 py-2">分類 / 処理</th><th className="px-3 py-2">知識</th><th className="px-3 py-2">メッセージ</th></tr></thead><tbody>{logs.data?.logs.map((log) => <tr key={log.log_id} className={`border-t align-top ${log.recovery_event ? 'bg-emerald-50' : log.incident_related ? 'bg-amber-50' : ''}`}><td className="whitespace-nowrap px-3 py-2 text-xs text-gray-600">{formatTime(log.received_at)}<br />{log.source_ip}</td><td className="px-3 py-2 font-medium">{log.hostname}</td><td className="px-3 py-2">S{log.severity}</td><td className="px-3 py-2 text-xs">{log.event_classification}<br /><span className="text-gray-500">{log.event_action ?? '-'}</span></td><td className="px-3 py-2 text-xs">{log.knowledge_status}<br /><span className="font-mono text-gray-500">{log.knowledge_id ?? '-'}</span></td><td className="max-w-xl px-3 py-2 font-mono text-xs break-words"><span className="mr-2 font-sans font-semibold">{log.recovery_event ? <span className="text-emerald-700">[復帰イベント]</span> : log.incident_related ? <span className="text-amber-700">[インシデント取込済み]</span> : <span className="text-gray-400">[未取込]</span>}</span>{log.message}<div className="mt-1 text-gray-500">{log.normalized_signature ?? ''}</div></td></tr>)}</tbody></table>
