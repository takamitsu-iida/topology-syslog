...
class HypothesisScorer:
    ...
    def _redundancy_penalty(self, candidate: str, observations: tuple[Observation, ...]) -> float:
        ...
        all_sessions = {
            ...
            if self._topology.object_type(successor) in {"bgp-session", "ospf-session", "lacp-session"}
        }
        covered_sessions = {
            observation.observed_object for observation in observations
            if self._topology.object_type(observation.observed_object) in {"bgp-session", "ospf-session", "lacp-session"}
        }
